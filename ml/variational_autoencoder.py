...
def load_gestures(path="dataset", sizeX=64, sizeY=64):
    gestures = {}
    for fname in os.listdir(path):
        if not fname.startswith('.'): ##against DS_Store shit
            with Image.open(os.path.join(path, fname)) as raw:
                img = raw
                img = raw.resize((sizeX, sizeY), Image.ANTIALIAS)
                img = np.divide(np.array(img), 255)
                gestures[fname] = img
    return gestures


def plot_random_gestures(decoder, n_to_show=10):
